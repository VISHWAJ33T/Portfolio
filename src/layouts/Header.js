...
export default function Header({ darkModeEnabled, toggleMode }) {
  ...
  return (
    <header className="container w-fit mx-auto">
      <nav
        className={`fixed top-0 right-0 z-[1000] flex h-16 w-full items-center justify-between px-4 md:h-16 md:px-[4%] ${isScroll ? 'backdrop-blur-xl backdrop-filter' : ''
          }`}
      >
        <motion.div
